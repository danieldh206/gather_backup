...
def main():
    ...
    config_filenames = glob.glob('/usr/groups/netops/data/config-backup/network-device-configs/Solarwinds/K20/edmonds-ctc.cfg')
...
def yaml_file(data,filename):
    ...
    for match in ['interfaces', 'roles', 'routing']:  # insert newlines before these keys
        yaml_str = re.sub(r'(\n' + match + r':)', r'\n\1', yaml_str, count=1)
    with open(f'/usr/lusers/danieldh/python_projects/Ansible/python_scripts/{filename}', 'a') as f:
        f.write(yaml_str)
