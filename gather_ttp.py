...
def get_vlans(conf):
    ...
    ifc_lines = conf.find_objects_w_child(r'^\s*service instance 1 ethernet', r'^\s*encapsulation dot1q (\d+)')
    print (ifc_lines, 'line 155')
        
    # vlans = (ifc_lines)
    # parser_vlan = ttp(data=vlans, template=ttp_template_vlans)
    # parser_vlan.parse()
    # vlans = parser_vlan.result(format="raw")[0]
    # print (vlans)
    ...
    for ifc_line in ifc_lines:
        # print (ifc_line)
        name = ifc_line.re_match(r'^\s*interface (.+)\s*$', default = 'te0/0/3')
        # print (name)
...
def get_roles(conf):
    ...
    for ifc_line in ifc_lines:
        ...
        if ifc_line.re_search_children(r'^\s*shutdown\s*$'):
           ...
           or ifc_line.re_search_children(r'^\s*ipv6 address 2607:FA78:1:')
...
def match_roles(ifc_roles, ifc_ips, vlans):
    ...
    for ifc, role in ifc_roles.items():
        ...
        try:
            ...
            continue
        if role in ['uplink', 'logical uplink', 'physical uplink','dmz','ups']:
...
def parse_route_v4(route_line):
    ...
    if v4:
        ...
        parser_v6.parse()
        v6 = parser.result(format="raw")[0]
        # network, interface, nexthop,  tag = v6[0]6
